...
unsigned long previouse_millis_1=0;
int lastpress1=0,lastpress2=0;
int toggle=0;
...
void loop()
{
  ...
  if(current_millis-previouse_millis_1>=500)
  {
    previouse_millis_1=current_millis;
    digitalWrite(4,toggle);
    toggle=!toggle;
  }
  if(digitalRead(5)<lastpress1)
  {
    if(rpmpulse>0)
      rpmpulse=rpmpulse*-1;
  }
  if(digitalRead(7)<lastpress2)
  {
    if(rpmpulse<0)
      rpmpulse=rpmpulse*-1;
  }
  lastpress1=digitalRead(5);
  lastpress2=digitalRead(7);
  //Serial.print(lastpress1);
  //Serial.print(lastpress2);
  //Serial.print(" ");
  if(Serial.available())
  {
